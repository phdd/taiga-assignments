          ...
          .map(project => {
            ...
            return request(openUserStoriesFor(project))
              .then(userstories => Promise.all(userstories
                .map(story => {
                  data[project.name]['stories'][story.subject] = []

                  return request(openTasksFor(story))
                    .then(tasks => Promise.all(tasks
                      .filter(task => task.assigned_to === me.id)
                      .map(task =>
                        data[project.name]['stories'][story.subject].push(task.subject)
                      )))

                    .then(() => {
                      // delete story if it neither contains tasks for me nor is assigned to me
                      if (data[project.name]['stories'][story.subject] &&
                          data[project.name]['stories'][story.subject].length === 0 &&
                          story.assigned_to !== me.id
                      ) {
                        delete data[project.name]['stories'][story.subject]
                      }
